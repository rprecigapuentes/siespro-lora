...
//ESP32 LoRa pins (adjust to your wiring)
#define LORA_SCK   18                           //SCK on ESP32 (VSPI SCK)
#define LORA_MISO  19                           //MISO on ESP32 (VSPI MISO)
#define LORA_MOSI  23                           //MOSI on ESP32 (VSPI MOSI)

#define NSS        5                            //select pin on LoRa device (CS)
...
void setup()
{
  ...
  SPI.begin(LORA_SCK, LORA_MISO, LORA_MOSI, NSS);   //initialise SPI with explicit pins for ESP32
  ...
  else
  {
    ...
    while (1)
    {
      delay(2000);                                  //avoid watchdog reset on ESP32
    }
  }
